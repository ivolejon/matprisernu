...
exports.format = function(data) {
    ...
    _.each(storeNames, function(store) {
        var tempPrice = calculate.calculateTotal(dataset.products, store);
        if(tempPrice < dataset.cheapest){
            dataset.cheapest = tempPrice;
            dataset.cheapestStore = store;
        }
        dataset.totals['total'+store] = tempPrice;
        dataset.totals['total'+store+'Shipping'] = dataset.totals['total'+store] + dataset.shipping[store];
    });
